...
def find_closest_node(graph: Graph, point: Point) -> int:
    ...
    for node, data in graph.nodes(data=True):
        lat, lon = data['pos']
        distance = haversine.haversine((point.lat, point.lon), (lat, lon))
...
def contains_node(monument_nodes: set, shortest_paths) -> bool:
    """Check if any monument node is present in the shortest paths."""
    ...
    for monument_node in list(monument_nodes):
        for path in list(shortest_paths[1].values()):
            if monument_node in path:
                ...
                route_graph = add_nodes_and_edges(G, path)

    return route_graph


def add_nodes_and_edges(G: Graph, path) -> Graph:
    """Add nodes and edges to the route graph."""
    route_graph = nx.Graph()
    for i in range(len(path) - 1):
        u, v = path[i], path[i + 1]
        point1 = get_node_position(G, u)
        point2 = get_node_position(G, v)
        if point1 and point2:
            lat1, lon1 = point1
            lat2, lon2 = point2
            weight = haversine.haversine((lat1, lon1), (lat2, lon2))
