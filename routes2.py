...
from monuments import Monument, load_monuments  # Asegúrate de que estos están definidos
from fastkml import KML, Document, Folder, LineString
...
def find_routes(graph: nx.Graph, start: Point, endpoints: Monuments) -> Routes:
    ...
    for endpoint in endpoints:
        try:
            route = nx.shortest_path(graph, source=(start.lat, start.lon), target=(endpoint.location.lat, endpoint.location.lon))
            routes.append(route)
...
def export_KML(routes: Routes, filename: str) -> None:
    ...
    for path in routes.path:
        ...
        line_string.geometry = line
        line_string.style = {"color": "blue", "width": 4}
