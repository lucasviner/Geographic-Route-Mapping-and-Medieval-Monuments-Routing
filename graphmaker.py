...
def make_graph(segments: list[Segment], num_clusters: int) -> nx.Graph:
    ...
    points = segments_to_numpy(segments)
    
    kmeans = KMeans(num_clusters)
    kmeans.fit(points) 
    centroids = kmeans.cluster_centers_  # (#centroide, latitud, longiut)
    labels = kmeans.labels_   # list of integers indicating to which centroid each point belongs
    graph = nx.Graph()
...
def segments_to_numpy(segments: list[Segment]) -> np.ndarray:
    """ Convert a list of segments to a numpy array of segments. """
    points: list[Point] = []
...
def create_edges(labels: np.ndarray) -> list[tuple[int,int]]: 
    ...
    adjency_matrix: list[list[int]] = [[0 for _ in range(max_cluster)] for _ in range(max_cluster)]
    
    # Fill the adjacency matrix with the connections between clusters
    for i in range(0, len(labels), 2):
        if labels[i] != labels[i + 1]:
            cluster1, cluster2 = labels[i], labels[i + 1]
            adjency_matrix[cluster1][cluster2] += 1
    ...
    for cluster1 in range(max_cluster):
        for cluster2 in range(max_cluster):
            if adjency_matrix[cluster1][cluster2] + adjency_matrix [cluster2][cluster1] >= 2:
...
def simplify_graph(graph: nx.Graph, epsilon: float) -> nx.Graph:
    ...
    nodes = nodes_to_remove(graph, epsilon)
    print(nodes)
    simplified_graph = graph.copy()
    
    for node, neighbor1, neighbor2 in nodes:
...
def nodes_to_remove(graph: nx.Graph, epsilon: float) -> list[tuple[int, int, int]]:
